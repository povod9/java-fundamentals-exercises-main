...
public class CrazyGenerics {
    ...
    static class CollectionUtil {
        ...
        public static boolean isValidCollection(Collection<? extends BaseEntity> entities, Predicate<? super BaseEntity> validationPredicate) {
            ...
            return entities.stream().allMatch(validationPredicate::test);
        }
        ...
        public static <T extends BaseEntity> T findMostRecentlyCreatedEntity(Collection<T> entities){
           return findMax(entities, CREATED_ON_COMPARATOR).orElseThrow(() -> new NoSuchElementException("asd"));
        }
        ...
        private static <T> List<T> swapHelper(List<T> elements, int i, int j){
            T s = elements.get(i);
            T t = elements.get(j);
            elements.set(j, s);
            elements.set(i, t);
        ...
        public static void jjj(List<? extends Number> nums){
