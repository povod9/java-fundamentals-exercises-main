...
public class CrazyStreams {
    ...
    public List<Account> findAccountsByBirthdayMonth(Month birthdayMonth) {
        return accounts.stream().filter(account -> account.getBirthday().getMonth().equals(birthdayMonth)).toList();
    }
    ...
    public boolean containsAccountWithEmailDomain(String emailDomain) {
        return accounts.stream()
                .anyMatch(a -> a.getEmail().split("@")[1].contains(emailDomain));
    }
    ...
    public Map<String, Set<String>> groupFirstNamesByLastNames() {
        return accounts.stream()
                .collect(groupingBy(Account::getLastName, Collectors.mapping(Account::getFirstName, toSet())));
    }
